...
import { AsyncValue } from '../../utils/utils.js'
...
Page({
  ...
  async call(e) {
    ...
    try {
      ...
      const { callerCameraStatus, listenerCameraStatus } = options
      ...
      const resp = await wmpfVoip.initByCaller({
        caller: {
          ...
          id: _caller.id,
          cameraStatus: Number(callerCameraStatus),
        },
        listener: {
          name: target.name,
          id: target.id,
          cameraStatus: Number(listenerCameraStatus),
        },
        roomType: roomtype,
